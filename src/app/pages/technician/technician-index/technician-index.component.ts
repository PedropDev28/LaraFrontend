    ...
    query(
      ...
      [
        style({
          opacity: 0,
          transform:
            'perspective(1000px) rotateY(10deg) translateX(50px) translateZ(-100px)',
        }),
        stagger(100, [
          animate(
            ...
            style({
              opacity: 1,
              transform:
                'perspective(1000px) rotateY(0) translateX(0) translateZ(0)',
            })
          ),
        ]),
      ],
...
export class TechnicianIndexComponent {
  ...
  ngOnInit() {
    this.authService.usuario$.subscribe(
      (usuario) => (this.usuarioConectado = usuario)
    );
    this.dbService
      .getUsersByParent(this.usuarioConectado.mail)
      .subscribe((data: any) => {
        this.users = data;
        for (let i = 0; i < this.users.length; i++) {
          this.dbService
            .getAudiosByUser(this.users[i].mail)
            .subscribe((data: any) => {
              console.log(data);
              if (data.length > 0) {
  ...
  setActiveTab(tab: 'phrases' | 'users') {
    this.activeTab = tab;
    this.cdr.detectChanges(); // Forzar detección de cambios
  }

  filterPhrasesLike(): void {
    ...
    if (searchInput) {
      console.log(this.phrases);
      this.phrases = this.phrases[0].filter((phrase:any) =>
        phrase.texto.texto
          .toLowerCase()
